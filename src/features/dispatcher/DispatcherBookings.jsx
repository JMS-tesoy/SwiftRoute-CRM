...
export default function DispatcherBookings({ parcels, drivers, onAssign }) {
  ...
  const filt = parcels.filter((p) => {
    ...
    const matchesSearch =
      !q ||
      p.trk.includes(q.toUpperCase()) ||
      p.recipient.toLowerCase().includes(searchTerm) ||
      p.merchant.toLowerCase().includes(searchTerm)

    return matchesSearch && (fs === 'all' || p.status === fs)
  })
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 14, flexWrap: 'wrap' }}>
        ...
        >
          ...
          <input
            ...
            placeholder="Search tracking, recipient, merchant…"
            style={{
              background: 'none',
              border: 'none',
              outline: 'none',
      ...
      <Card style={{ overflow: 'hidden' }}>
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: '120px 110px 1fr 150px 120px 90px',
            padding: '9px 16px',
        ...
        {filt.map((p, i) => {
          ...
          return (
            ...
            >
              ...
              <div>
                <div style={{ color: C.text, fontSize: 12.5, fontWeight: 500 }}>
                  {p.recipient}
                </div>
              ...
              <span style={{ color: p.driver ? C.text : C.dimText, fontSize: 12 }}>
                {p.driver || '—'}
              </span>

              <div>
                {['pending', 'assigned'].includes(p.status) && (
                  <Button onClick={() => setTgt(p)}>
                    <Truck size={10} />
                    {p.driver ? 'Reassign' : 'Assign'}
                  </Button>
                )}
              </div>
            </div>
